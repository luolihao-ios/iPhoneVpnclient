...
- 三端统一使用 `0.1.3+4` 版本号；Windows 安装包和 Android APK 按此版本命名，iOS IPA 由 GitHub Actions 生成。
- iOS Packet Tunnel 新增独立路由诊断日志，避免 AnyTLS 高频日志覆盖 DNS/路由信息。
- 智能分流中的海外域名使用代理 DoH，国内域名和中国 IP 使用规则集直连；三组远程 DoH 解析器支持按上次成功项优先自动切换。
