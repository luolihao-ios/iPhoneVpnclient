...
- 本项目新增或修改的 Markdown 文档、设计说明、实施计划、项目总结、发布说明和面向用户的技术文案，默认使用中文。
- 仅当外部平台、协议字段、代码标识符或用户明确要求英文时，才保留必要英文；其余说明必须提供中文。
- 这项约定适用于后续所有迭代，不因切换 Codex 任务或新对话而失效。

### 29. Clash Hysteria / AnyTLS 订阅兼容（2026-08-16，0.1.5.004）

- 共享订阅解析器新增旧版 Clash `hysteria` 节点支持，并补齐 Hysteria2 的 `ports`、`server-ports`、`hop-interval` 等端口跳跃字段；Windows、Android 和 iOS 均复用同一实现。
- Hysteria/Hysteria2 生成 sing-box 配置时，存在端口范围就只写入 `server_ports`，避免与 `server_port` 同时存在导致核心配置冲突；旧版 Hysteria 同时映射 `auth_str`、混淆、带宽和 TLS 字段。
- AnyTLS 兼容 Clash 的 `server-port`、`skip-cert-verify`、会话检查间隔、会话超时和最小空闲会话等连字符字段，减少导入后遗漏节点或连接参数不完整的情况。
- Clash YAML 导入日志新增脱敏解析统计：仅记录正文长度、短指纹及各协议类型的源数量、接受数量和跳过数量；不记录订阅地址、服务器、密码、UUID 或令牌，可直接定位 Forge 与其他客户端的解析数量差异。
- 三端可识别构建号更新为 `0.1.5.004`。

### 28. 空 HTTP 200 订阅响应兼容（2026-08-16，0.1.5.002）
- 部分订阅服务会对未知客户端标识返回 HTTP `200` 但正文为空；此前客户端将其作为空订阅解析，导致界面提示成功但节点数为零。
- 订阅请求现在在收到空 `200` 响应时也会以 `flclash` 标识重试，兼容按客户端标识分发 Clash YAML 的服务；新增回归测试覆盖此场景。
- 三端统一升级为 `0.1.5+2`，设置页构建标记为 `0.1.5.002`。
...
- 删除远程 DNS 首选项持久化、三组切换协调器及统一失败提示；正式发布版本为 `0.1.4+1`，设置页可识别构建号为 `0.1.4.001`。
...
- 仅收到 HTTP 204 才判定可用；HTTP CONNECT 200、普通 HTTP 200、重定向、错误响应和超时均不再误报为可用。
- 临时 sing-box 启动和 HTTP 204 响应共用单节点 3 秒总时限，继续遍历全部节点并保留手动停止功能。
- Android 与 iOS 继续使用节点 TCP 检查，不受 Windows HTTP 204 规则影响。
- 本轮可识别构建号更新为 `0.1.2.003`。

### 23. Windows 全节点连通性检查与关机代理恢复（2026-08-11）

- 自动检查和手动检查都会遍历全部节点，不再按可用节点数量或整批时间提前结束。
- 保留全节点 TCP 预筛；TCP 可达后，通过临时 sing-box 向 `www.youtube.com:443` 建立 HTTP CONNECT 隧道，收到 `200 Connection established` 即判定可用，不下载页面正文，也不依赖第三方出口 IP 查询服务。
- 单节点完整验证上限统一为 3 秒，仍采用 3 路并发并支持手动停止；停止后保留已完成结果。
- Windows 原生窗口处理 `WM_QUERYENDSESSION` 和 `WM_ENDSESSION`，在注销、重启或关机时同步恢复 Forge VPN 接管前的系统代理；窗口销毁时再次执行兜底恢复。
- `ProxyEnable=0` 时保留其他客户端留下的禁用代理地址，不把无效地址残留误判为代理仍在启用。
- 本轮可识别构建号更新为 `0.1.2.002`。
